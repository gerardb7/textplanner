...
public class BabelNetDictionary implements MeaningDictionary
{
	...
	public Optional<String> getLabel(String id, ULocale language)
	{
		try
		{
			...
			if (sense == null)
				return Optional.empty();
			return Optional.of(sense.getSimpleLemma().toLowerCase(Locale.ENGLISH));
		}
		catch (Exception e)
		{
			log.warn("BabelNet error: " + e);
		...
		try
		{
			...
			while (it.hasNext())
			{
				...
				if (item.getLanguage().equals(bnLang))
				{
					...
					final Pair<String, POS.Tag> word_tag = Pair.of(item.getWord().toLowerCase(Locale.ENGLISH), POS.BabelNet.get(item.getPOS().getTag()));
