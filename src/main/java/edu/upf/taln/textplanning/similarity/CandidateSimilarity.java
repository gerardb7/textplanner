...
public class CandidateSimilarity
{
	...
	private static final double default_sim_value = 0.5;
	...
	public double computeSimilarity(Candidate c1, Candidate c2)
	{
		...
		double semantic_sim = sem.computeSimilarity(c1.getEntity(), c2.getEntity()).orElse(default_sim_value);

		double closeness = 0.0;
		LinguisticStructure s1 = c1.getMention().getStructure();
		Document d1 = s1.getDocument();
		LinguisticStructure s2 = c2.getMention().getStructure();
