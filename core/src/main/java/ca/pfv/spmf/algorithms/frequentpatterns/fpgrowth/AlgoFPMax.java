...
public class AlgoFPMax
{
	 ...
	 *
	 * @param inTransactions transaction database.
	 * @param inMinSupport the minimum support threshold.
	 * @return the result
	 */
	public Itemsets runAlgorithm(List<Set<Integer>> inTransactions, int inMinSupport)
	{
		this.minSupportRelative = inMinSupport;
		patterns = new Itemsets("FREQUENT ITEMSETS");
		mfiTree = new MFITree();

		// (1) PREPROCESSING: Initial database scan to determine the frequency of each item
		originalMapSupport = scanDatabaseToDetermineFrequencyOfSingleItems(inTransactions);
		...
		for (Set<Integer> t : inTransactions)
		{
			List<Integer> filteredTransaction = new ArrayList<Integer>();
		...
		if (tree.headerList.size() > 0)
		{
			...
			fpMax(tree, itemsetBuffer, 0, inTransactions.size(), originalMapSupport);
		}
...
//		======= DEBUG ========
	 ...
	 *
	 * @param inTransactions input transactions
	 * @return a map for storing the support of each item (key: item, value: support)
	 */
	private Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItems(List<Set<Integer>> inTransactions)
	{
